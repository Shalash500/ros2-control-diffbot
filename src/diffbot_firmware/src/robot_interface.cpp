#include <diffbot_firmware/robot_interface.hpp>
...
CallbackReturn RobotInterface::on_init(const hardware_interface::HardwareInfo &hardware_info)
{
    ...
    {
        RCLCPP_FATAL_STREAM(rclcpp::get_logger("RobotInterface"), "No serial port provided Aborting");
...
std::vector<hardware_interface::CommandInterface> RobotInterface::export_command_interfaces()
{
    ...
    for(size_t i = 0; i < info_.joints.size(); i++)
    {
        command_interfaces.emplace_back(hardware_interface::CommandInterface(info_.joints.at(i).name, hardware_interface::HW_IF_VELOCITY, &velocity_commands_.at(i)));
    }
...
CallbackReturn RobotInterface::on_deactivate(const rclcpp_lifecycle::State &previous_state)
{
    ...
    if (arduino_.IsOpen())
    {
        ...
        {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("RobotInterface"), "Something went wrong while closing the port" << port_);
            return CallbackReturn::FAILURE;
        }
...
hardware_interface::return_type RobotInterface::write(const rclcpp::Time &time, const rclcpp::Duration &period)
{
    ...
    {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("RobotInterface"), "Something went wrong while sending the message " << message_stream.str() << " on the port " << port_);
